...
function removeLastOccurrences(str, value) {
  ...
  if (lastIndex !== -1) {
    // Use substring to remove the last occurrence
    return (
...
function formatTime(minutes, seconds) {
  ...
  // Concatenate formatted minutes and seconds with ":"
...
function countVowels(str) {
  ...
  // Use the split and filter methods to count the vowels
  const vowelCount = str.split('').filter((char) => vowels.has(char)).length;
...
function isPalindrome(str) {
  ...
  // Use the split, reverse, and join methods to check if it's a palindrome
...
function invertCase(str) {
  // Use a regular expression to replace each character with its inverted case
  const invertedString = str.replace(/[a-zA-Z]/g, (char) => {
    // Check if the character is uppercase and return the inverted case
...
function extractNameFromTemplate(value) {
  // Use a regular expression to extract the first and last names
...
function extractEmails(str) {
  // Use the split method to separate e-mails based on semicolons
  const emailList = str.split(';').map((email) => email.trim());

  // Filter out empty strings (in case there are consecutive semicolons)
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, function rot13Char(char) {
...
function getCardId(value) {
  ...
  // Find the index of the specified card in the initial deck
